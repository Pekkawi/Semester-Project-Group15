...
void display_distance(unsigned int , float);
...
display_speed(relative_speed);
display_distance(time,relative_speed);
}
...
void set_speed_phase_one(){
/*float a = 255*1.0*(22*1.0 /time_phase_one*1.0);//255 *(22/25)
...
void set_speed_phase_two(){
  ...
  //alternative code if the car was working int b = 255*1.0*(max_time*1.0/time_phase_two*1.0);
  OCR0A = 255;
}
unsigned int buffer_reader(char a[10]){
...
void display_distance(unsigned int time,float speed){
float real_distance = (time*1.0 / 1000.0)*speed;
distance_went+=real_distance;
int int_distance= (int)(distance_went*10);
printf("page2.x1.val=%d%c%c%c",int_distance,255,255,255);
}

void display_speed(float speedo){
 int e = (int)(speedo*100);
printf("page2.x2.val=%d%c%c%c",e,255,255,255);
}